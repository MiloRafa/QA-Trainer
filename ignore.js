...
var server = http.createServer(function (request, response){
    ...
    // if (path === "/") {
    // fs.readFile("./Trainers.html", function (error, data) {
...
<form action = 'http://localhost:8000/trainers' method='post'>
    ...
    /* if (path === "/trainers") {
         ...
         connection.query("select * from trainers", function (err, results) {
             if (err) {
                 console.log(err);
             }
             else {
                 ...
                 results.forEach(function (rec) {
                     response.write(`<a href='http://localhost:8001/trainersinfo'> ${rec.Name}</a>`);
                 });
                 response.end();
             }
         });
